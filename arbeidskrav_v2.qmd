...
# Anta at datasettet ditt er lastet inn som ny_data
# Rydde opp i avstandene (gjør dette først)
ny_data <- ny_data %>%
  mutate(avstand_km = map_dbl(`Estimer i antall km hvor lang din reise er til jobb.`, rydd_avstand))
...
# Filtrer ut avstander over 100 km og lag en frekvensfordeling for de ulike transportmåtene
frekvensfordeling <- ny_data_separated %>%
  filter(avstand_km <= 100) %>%  # Filtrer for avstander under eller lik 100 km
  group_by(`Hvordan kommer du deg til jobb`) %>%
  summarise(antall = n(),
            Gjennomsnittlig_avstand = mean(avstand_km, na.rm = TRUE)) %>%
  arrange(desc(antall))
